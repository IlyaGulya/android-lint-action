...
export const convertLintToCheckstyle = (
  ...
  return Effect.gen(function* () {
    ...
    yield* pipe(
      ...
      Effect.flatMap(issues => buildCheckstyleXml(issues, getDefaultConfig())),
...
const impl: XmlConverter = {
  convertLintToCheckstyle: convertLintToCheckstyle,
};

export const XmlConverter =
  Context.GenericTag<XmlConverter>("ActionXmlConverter");

export const layer = Layer.succeed(XmlConverter, impl);

export function makeNoop(impl: Partial<XmlConverter>): XmlConverter {
  return {
    convertLintToCheckstyle: () => Effect.fail(new Error("not implemented")),
    ...impl,
  };
}

export function layerNoop(
  impl: Partial<XmlConverter> = {},
): Layer.Layer<XmlConverter> {
  return Layer.succeed(XmlConverter, makeNoop(impl));
}
